...
def detect(opt):
    ...
    #                        print(output)
                            ...
                            # if not(id in mc_ids) and cls == 3:
                            #     mc_ids[id] = 1
                            #     mc_count += 1
                            #     print(f'mc_ids = {mc_ids}')

                            if cls == 3:
                                print(f'motorcycle detected: {output}')
                                if not(id in mc_ids):
                                    mc_ids[id] = 1
                                else:
                                    mc_ids[id] += 1
                                    if mc_ids[id] == opt.cthres:
                                        mc_count += 1
                                        print(f'mc_ids = {mc_ids}')

                            # FIXME: 화면에 10회 등장한 객체만 카운트 (FIXME: 10회 연속 등장으로 조건 바꾸기
                            # 연속등장하지 않은 객체는 딕셔너리에서 제거하기. 10회 이상)
                            # 10회 등장한 id는 done 딕셔너리로 옮기고 재등장 시 카운트하지 않기
                            ...
                            if save_txt:
                                ...
                                with open(txt_path, 'a') as f:
                                   ...
                                   if cls == 3:
                                       ...
                                       # print(label_format)
                                       f.write(f'{frame_idx}, {cls}, {label_format}')  # label format
            ...
            print(f'fps = {(frame_idx + 1) / elapsed},  mc_count = {mc_count}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--period', type=int, default=1, help='frame skip peroid')
    parser.add_argument('--cthres', type=int, default=5, help='threshold of occurance to count object')
